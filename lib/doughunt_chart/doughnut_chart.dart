...
/// 选中扇形区域后执行的回调
typedef DoughnutSelectCallback = void Function(DoughnutDataItem? selectedItem);

class Doughnut extends CustomPainter {
  ///圆心位置
  late Offset circleCenter;

  /// 选中的区域
  final DoughnutDataItem? selectedItem;

  /// 选中区域回调
  final DoughnutSelectCallback? brnDoughnutSelectCallback;
      ...
      if (showTitleWhenSelected == false ||
        ...
        Offset baseRectCenter = Offset(
            indicatorEndOffset.dx > revisedIndicatorRPoint.dx
                ? indicatorEndOffset.dx + textWidth / 2 + textHorizontalPadding
                : indicatorEndOffset.dx - textWidth / 2 - textHorizontalPadding,
...
class BrnDoughnutChart extends StatelessWidget {
  ...
  /// 选中项目时候的回掉
  final DoughnutSelectCallback? selectCallback;
