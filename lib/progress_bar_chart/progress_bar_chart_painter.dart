...
/// 每一个bundle对应一组数据
class ProgressBarBundle {
  final List<ProgressBarItem> barList;
...
class ProgressBarChartPainter extends CustomPainter {
  ...
  void _prepareData(Canvas canvas, Size size) {
    if (yAxis.axisItemList.isNotEmpty) {
      ...
      yAxisRect = Rect.fromLTWH(0, 0, yAxis.maxTextWidth + _yTextAxisSpace,
          size.height - (xAxis.axisItemList.isEmpty ? 0 : _xAxisHeight));
    }

    if (!drawY) {
      yAxisRect = Rect.fromLTWH(0, 0, 0,
          size.height - (xAxis.axisItemList.isEmpty ? 0 : _xAxisHeight));
    }

    xAxisRect =
        Rect.fromPoints(yAxisRect.bottomRight, Offset(size.width, size.height));

    if (barChartStyle == BarChartStyle.horizontal) {
      if (!drawX) {
        // 仅画x
        xAxisRect = Rect.fromLTWH(yAxisRect.bottomRight.dx, size.height,
            size.width - yAxisRect.width, 0);
        yAxisRect = Rect.fromLTWH(0, 0, yAxis.maxTextWidth + _yTextAxisSpace,
            size.height - (xAxis.axisItemList.isEmpty ? 0 : _xAxisHeight));
      }
    }
    ...
    if (barBundleList.isNotEmpty) {
      contentRect = Rect.fromPoints(yAxisRect.topRight, xAxisRect.topRight);
    }
    ...
    if (0 != barMaxValue) {
      ...
      barBundleList.forEach((ProgressBarBundle barbundle) {
        barbundle.barList.forEach((ProgressBarItem barItem) {
    ...
    barItemEnumerator((int barBundleIndex, ProgressBarBundle barBundle,
      ...
      if (BarChartStyle.horizontal == barChartStyle) {
        ...
        Offset leftTop = Offset(
            yAxisRect.right,
            (barBundleCount * singleBarWidth + barGroupSpace) * barGroupIndex +
                yAxis.leadingSpace);
    ...
    if (BarChartStyle.horizontal == barChartStyle) {
      ...
      for (int yAxisItemIndex = 0;
        ...
        _drawDashLineOn(
            canvas,
            yAxisItemOffset,
            Offset(yAxisItemOffset.dx + contentRect.width, yAxisItemOffset.dy),
  ...
  void barItemEnumerator(BarItemEnumeratorCallback barItemEnumeratorCallback) {
    ...
    for (int barBundleIndex = 0;
        ...
        barBundleIndex++) {
      ProgressBarBundle barBundle = barBundleList[barBundleIndex];
      int barNumber = barBundle.barList.length;
      for (int barGroupIndex = 0; barGroupIndex < barNumber; barGroupIndex++) {
        ProgressBarItem barItem = barBundle.barList[barGroupIndex];
    ...
    if (null != selectedBarItem) {
      ...
      TextPainter selectedBarTextPainter = TextPainter(
          text: TextSpan(
              ...
              style: TextStyle(fontSize: 12, color: selectedHintTextColor)),
          textDirection: TextDirection.ltr)
        ..layout(maxWidth: double.infinity, minWidth: 0);
        ...
        if (brnProgressBarChartSelectCallback != null &&
          ...
          if (onBarItemClickInterceptor == null ||
            ...
            brnProgressBarChartSelectCallback!(
                barItem.barRect == selectedBarItem?.barRect ? null : barItem);
          }
        }
      });
    }
  ...
  @override
  bool shouldRepaint(ProgressBarChartPainter oldDelegate) {
    return true;
  }
}
